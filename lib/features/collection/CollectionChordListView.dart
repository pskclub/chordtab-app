...
import 'package:chordtab/features/favorite/ChordFavoriteItemView.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';

import 'CollectionChordItemView.dart';

class CollectionChordListView extends StatelessWidget {
  final List<ChordItemModel> items;
  final bool isItemRounded;

  const CollectionChordListView({Key? key, required this.items, this.isItemRounded = false}) : super(key: key);

  CollectionChordItemView _buildItemsForListView(BuildContext context, int index) {
    return CollectionChordItemView(
  ...
  showAlertDialog(BuildContext context, ChordItemModel item) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      content: Text("ลบออกจากคอลเลกชั่น?"),
      actions: [
        confirmButton,
        cancelButton,
      ],
